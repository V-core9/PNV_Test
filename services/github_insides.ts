...
export const createGithubInsides = (auth) => {
  ...
  insides.getRateLimit = async () => {
    const { data } = await octokit.rest.rateLimit.get();
    return data;
  };


  insides.getUserInfo = async ({ username }) => {
  ...
  insides.getUserRepos = async ({ user, page = 1, per_page = 50 }) => {
  ...
  insides.gatherUsersLanguages = async (username: string) => {
    username = String(username).toLowerCase();
