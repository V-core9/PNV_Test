  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: "1em", padding: ".25em 1em", border: '1px dashed #10203040', }}>
        {page > 1 && <>
          <Link href={`/users/?page=1${props.perPage !== undefined ? '&perPage=' + perPage : ''}`}>⏪ First</Link>
          <Link href={`/users/?page=${page - 1}${props.perPage !== undefined ? '&perPage=' + perPage : ''}`}>◀ Prev</Link>
        </>}
        <Link href="#">{page}</Link>
        {page < totalPages && <>
          <Link href={`/users/?page=${page + 1}${props.perPage !== undefined ? '&perPage=' + perPage : ''}`}>Next ▶</Link>
          <Link href={`/users/?page=${totalPages}${props.perPage !== undefined ? '&perPage=' + perPage : ''}`}>Last ⏩</Link>
        </>}
        Total {total}
